<!-- Last verified: 2026-07-20. Update this file whenever a new public type/function/component is added or removed from the frontend. Check here before adding new code — don't duplicate something that already exists. -->
...
| `parentPortalApi` | `parentPortal.ts` | Parent-facing read surface (Parent role). `getChildren()`, `getAcademicYears()`, `getChildGrades(childId, academicYearId?)`, `getChildAttendance(childId, academicYearId?)` → `ParentAttendance` (`{ summary, entries }`), `getChildFees(childId, academicYearId?)` → `StudentFeeOverview` (`{ summary, invoice }`) (year optional → backend defaults to current). Hits backend `/parent/*`. |
