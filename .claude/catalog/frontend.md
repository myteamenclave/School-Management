...
| `parentPortalApi` | `parentPortal.ts` | Parent-facing read surface (Parent role). `getChildren()`, `getAcademicYears()`, `getChildGrades(childId, academicYearId?)`, `getChildAttendance(childId, academicYearId?)` → `ParentAttendance` (`{ summary, entries }`) (year optional → backend defaults to current). Hits backend `/parent/*`. |
| `PARENT_KEYS` | `parentPortal.ts` | Query key factory: `{ children(), academicYears(), childGrades(childId, academicYearId), childAttendance(childId, academicYearId) }`. |
...
| `ParentRoutes` | `parent/index.tsx` | Nested `<Routes>` for `/parent/*`: `grades` → `ChildGradesPage`, `attendance` → `ChildAttendancePage`; `*` redirects to `grades`. |
