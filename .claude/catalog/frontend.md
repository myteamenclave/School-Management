<!-- Last verified: 2026-07-22. Update this file whenever a new public type/function/component is added or removed from the frontend. Check here before adding new code — don't duplicate something that already exists. -->
...
| `parentAccountsApi` | `parentAccounts.ts` | Thin wrappers over the student sub-resource: `list(studentId)`, `createLogin(studentId, body)`, `removeLink(studentId, parentUserId)`. |
| `PARENT_ACCOUNT_KEYS` | `parentAccounts.ts` | TanStack Query key factory: `{ forStudent(studentId) }`. Create/remove invalidate this key. |
...
| `parentPortalApi` | `parentPortal.ts` | Parent-facing surface (Parent role). Reads: `getChildren()`, `getAcademicYears()`, `getChildGrades(childId, academicYearId?)`, `getChildAttendance(childId, academicYearId?)` → `ParentAttendance`, `getChildFees(childId, academicYearId?)` → `StudentFeeOverview` (year optional → backend defaults to current). Writes (pay-online): `payInstallment(childId, installmentId)` → `InitiatePaymentResult`, `confirmPayment(childId, paymentId)` (return-path reconcile). Hits backend `/parent/*`. |
| `PARENT_KEYS` | `parentPortal.ts` | Query key factory: `{ children(), academicYears(), childGrades(childId, academicYearId), childAttendance(childId, academicYearId), childFees(childId, academicYearId) }`. |
| `useParentChildYear` | `parent/useParentChildYear.ts` | Shared parent-page bootstrap hook: loads children + years, auto-selects the only child, defaults to the current year. Returns selection state + `selectedChild`/`selectedYear`. Consumed by both parent pages. |
| `ParentChildYearBar` | `parent/ParentChildYearBar.tsx` | Shared parent control bar: child switcher (hidden for a single child) + academic-year `Select` + selected-child summary line. Presentational; selection lifted via props. |
| `AttendanceStatusBadge` / `ATTENDANCE_STATUS_COLORS` / `ATTENDANCE_STATUS_TEXT_COLORS` | `components/shared/AttendanceStatusBadge.tsx` | Shared attendance status chip + colour maps (Present green / Late amber / Absent red / Excused blue). Same values previously inlined in admin/teacher attendance pages; `*_TEXT_COLORS` is the text-only variant for numeric summaries. |
...
| `StudentFeeOverview` / `StudentFeeSummary` / `FeeInvoice` (parent) | `parentPortal.ts` | `StudentFeeOverview` = `{ summary, invoice }`. `StudentFeeSummary` = balance rollup (`hasInvoice`, `totalBilled`/`totalPaid`/`outstanding`, `nextDueDate`/`nextDueAmount`, `overdueAmount`/`overdueCount`, `overdueInstallmentIds`). `FeeInvoice` mirrors the admin `FeeInvoiceDto` (line items + installments), read-only. |
| `InitiatePaymentResult` | `parentPortal.ts` | `{ paymentId, clientSecret, publishableKey }` — returned by `payInstallment`, drives the Stripe Elements inline card form. |
...
| `gradeScaleApi` | `gradebook.ts` | Grade-scale band access: `getAll()` (Admin + Teacher — teacher gradebook reads it for live letter mapping), `create(body)`/`update(id, body)`/`remove(id)` (Admin-only). Backend `/grade-scale`. |
...
| `StudentDetailPage` | `admin/students/StudentDetailPage.tsx` | Admin detail page at `/admin/students/:id`. Back button row, then name + studentCode heading. Four shadcn `Tabs`: **Details** (edit form), **Section Assignments** (`StudentSectionAssignmentsTab`), **Fee Assignment** (`FeeAssignmentTab`), **Parent Accounts** (`ParentAccountsTab`). |
...
| `StudentDetailPage` | `admin/students/StudentDetailPage.tsx` | Four shadcn `Tabs`: **Details** (edit form), **Section Assignments** (`StudentSectionAssignmentsTab`), **Fee Assignment** (`FeeAssignmentTab`), **Parent Accounts** (`ParentAccountsTab`). |
...
| `ParentAccountsTab` | `admin/students/components/ParentAccountsTab.tsx` | "Parent Accounts" tab on `StudentDetailPage` (takes the full `StudentDto`). Lists linked parents (name/email/added date) via `parentAccountsApi.list`; remove-link with `window.confirm` (link-only, keeps the user). "Create parent login" button disabled with a hint when the student has no guardian email. Composes `CreateParentLoginModal`. |
| `CreateParentLoginModal` | `admin/students/components/CreateParentLoginModal.tsx` | shadcn Dialog with two states: state 1 shows the guardian email (read-only) + an **auto-generated** temporary password (`generatePassword()` via `crypto.getRandomValues`, copy + regenerate), posting `parentAccountsApi.createLogin`; state 2 is an in-modal credentials panel with copy-to-clipboard. Distinct variant when an existing Parent account is reused (`accountCreated=false` → no password shown, "password unchanged" note; `linkCreated=false` → "already linked"). |
...
| `ParentRoutes` | `parent/index.tsx` | Nested `<Routes>` for `/parent/*`: `grades` → `ChildGradesPage`, `attendance` → `ChildAttendancePage`, `fees` → `ChildFeesPage`; `*` redirects to `grades`. |
| `ChildGradesPage` | `parent/grades/ChildGradesPage.tsx` | Parent portal grades view at `/parent/grades`. Uses `useParentChildYear` + `ParentChildYearBar` for child/year selection. Report-card table grouped by semester (Subject · Term · Letter badge); component scores never rendered. Empty states: no children linked, no grades for child/year, loading, error. |
| `ChildAttendancePage` | `parent/attendance/ChildAttendancePage.tsx` | Parent portal attendance view at `/parent/attendance`. Uses `useParentChildYear` + `ParentChildYearBar`. Summary hero (attendance rate + Present/Late/Absent/Excused counts, coloured) over a reverse-chronological daily log (Date · Status badge · Section · Notes; sorted desc in-UI). Empty states: no children linked, no attendance recorded (`totalMarked === 0`), loading, error. |
| `ChildFeesPage` | `parent/fees/ChildFeesPage.tsx` | Parent portal fees view at `/parent/fees`. Uses `useParentChildYear` + `ParentChildYearBar`. Balance hero (Outstanding/Billed/Paid/Next-Due) + overdue callout when `overdueCount > 0`, over the installment schedule (rows flagged Overdue via `summary.overdueInstallmentIds` — never recomputed) and the line-item breakdown (Original/Discount/Final + total footer). Currency in `en-PH`/PHP (matches admin fee pages). Each non-Paid installment row has a **Pay** button → `PayInstallmentModal`; on success invalidates the `childFees` query. Empty states: no children linked, no Issued invoice (`!hasInvoice`), loading, error. |
| `PayInstallmentModal` | `parent/fees/PayInstallmentModal.tsx` | Stripe Elements **inline** payment modal (no redirect). On open, calls `payInstallment` to create the intent, then `loadStripe(publishableKey)` (memoised) + `<Elements options={{clientSecret}}>` wrapping `<PaymentElement/>`. Submit → `stripe.confirmPayment({ redirect: 'if_required' })`; on `succeeded` calls `confirmPayment` (return-path reconcile) then `onPaid`. Uses `@stripe/stripe-js` + `@stripe/react-stripe-js`; toasts via `sonner`. |
