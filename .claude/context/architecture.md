...
6. **Authorization** — role-based (Admin, Teacher, Principal — canonical name for "Principal/Owner", Parent), claims embedded in the access token JWT (incl. `school_id`); per-tenant scoping (`SchoolId`) is enforced separately via EF Core global query filters, not via the JWT. `ITenantProvider.CurrentSchoolId` (`HttpContextTenantProvider`) reads the `school_id` claim from the authenticated request — this is the real implementation that replaced spec #1's `StaticTenantProvider` placeholder, with no other code changes anywhere else (the payoff of that abstraction).
