...
Build a single-school system, architecturally ready for multi-tenancy, that serves all four real stakeholders a school management system implies: **admin/registrar** (full-depth CRUD and operations — this stays a "management" tool first), **parent** (portal with grades/attendance visibility and online fee payment), **principal/owner** (a revenue and operations dashboard), and **teacher** (attendance marking and a lightweight gradebook).

The brief named three nouns — students, staff, fees. The pitch-winning move is showing the connective tissue between them: a teacher marks attendance → a parent sees it and pays a fee online → a principal sees both reflected in a live dashboard. That end-to-end coherence is more convincing than a long, disconnected feature list, even though the feature list itself is also broader than what was asked for.
...
- [ ] One human engineer + AI agent can deliver admin + parent + principal + teacher surfaces at "reasonable depth" (per #7 domain lens: academic year/term, class/section, fee templates with installments/discounts, bulk import, audit trail) in 1-2 weeks — validate via daily scope checkpoints, not at the end.

## MVP Scope
**In:**
- Admin: student CRUD + bulk CSV import, staff CRUD, class/section assignment, academic year/term setup, fee structure templates (installments, discounts/scholarships), fee invoicing, financial audit trail
- Parent portal: view child's grades/attendance/fee balance, pay fees online (sandbox payment gateway)
- Principal dashboard: revenue vs. outstanding fees, enrollment trend, attendance trend, staff overview
- Teacher: mark daily attendance, enter grades per subject/term (no timetable engine, no weighted GPA)
- Auth/RBAC across all four roles
...
- How much detail does the principal dashboard need (live data vs. seeded snapshot) given it's built last and depends on every other module having real data?
