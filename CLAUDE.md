...
| [docs/ideas/13-overview-dashboard.md](docs/ideas/13-overview-dashboard.md) | Admin overview dashboard — replaces landing page, signal-over-vanity (finance hero + exceptions, drill-through), single live aggregation endpoint, shadcn/Recharts, monthly buckets, academic-year selector; enrollment as breakdown not trend, teachers as coverage snapshot |
| [docs/ideas/15-parent-portal-grades.md](docs/ideas/15-parent-portal-grades.md) | Parent Portal — view child's grades: first parent-facing read surface, JWT-scoped authorization (children derived from `StudentParent` links, never a trusted id), reusable parent shell (child switcher + year selector), report-card (Term+Letter) view reusing the spec-15 grade read |
| [docs/ideas/16-parent-portal-attendance.md](docs/ideas/16-parent-portal-attendance.md) | Parent Portal — view child's attendance: second parent read surface reusing the spec-18 shell + `ResolveLinkedChildOrThrow` guard, reuses spec-14 `GetStudentHistoryAsync`/`AttendanceHistoryEntryDto` for the daily log, adds a **server-computed** attendance summary (per-status counts + rate — rate formula as a business rule), second "Attendance" nav item + shared `ParentChildYearBar` |
| [docs/ideas/17-parent-portal-fees.md](docs/ideas/17-parent-portal-fees.md) | Parent Portal — view fee balance / invoices: fourth parent read surface reusing the parent shell + `ResolveLinkedChildOrThrow` guard, **Issued-only** invoice visibility, **server-computed** balance summary (Billed/Paid/Outstanding/Next-Due + **on-the-fly overdue** as business rules) reusing spec-12 `FeeInvoiceDto`, "Fees" nav item + shared `ParentChildYearBar`; no new entity/migration |
...
| [specs/16-implement-overview-dashboard.md](specs/16-implement-overview-dashboard.md) | Admin overview dashboard — single read-only `GET /api/dashboard/overview?academicYearId=` aggregation (no new entities/migration), `DashboardService` + read-model `IDashboardRepository`, finance/attendance/enrollment/teacher-coverage DTOs, `/dashboard` role-dispatch to `OverviewDashboard`, recharts + shadcn chart, `dashboard.ts` API client, year selector reusing academic-years list | Implemented |
| [specs/17-implement-parent-guardian-link.md](specs/17-implement-parent-guardian-link.md) | Parent–guardian link (backend) — `StudentParent` junction, `ParentAccountService`, Admin-only `POST /api/students/{id}/parent-login` (create-or-reuse Parent user from `GuardianEmail`, hash Admin-set temp password), `GET .../parents`, `DELETE .../parents/{parentUserId}` (link-only), `IUserRepository.FindByEmailInTenantAsync` addition, 409 on non-Parent email collision | Implemented |
| [specs/17-B-implement-parent-guardian-link-frontend.md](specs/17-B-implement-parent-guardian-link-frontend.md) | Parent–guardian link (frontend) — Student Detail "Parent Accounts" tab, `parentAccounts.ts` API client, `ParentAccountsTab` (list + remove-link with confirm) + `CreateParentLoginModal` (RHF/Zod temp-password form → in-modal credentials panel with copy, distinct reuse variant), create disabled when guardian email blank | Implemented |
| [specs/18-implement-parent-portal-grades.md](specs/18-implement-parent-portal-grades.md) | Parent Portal — view child's grades (backend + frontend): `api/parent` controller (Parent-only, all GET), `ParentPortalService` with single `ResolveLinkedChildOrThrow` guard (unlinked child → 404), `IStudentParentRepository.GetByUserIdAsync` addition, reuses `GradebookService.GetStudentGradesAsync`/`StudentGradeDto`; parent shell (`ParentRoutes` nested routes, nav item, child switcher, year selector) + report-card `ChildGradesPage`, `parentPortal.ts` API client; no new entity/migration | Implemented |
| [specs/19-implement-parent-portal-attendance.md](specs/19-implement-parent-portal-attendance.md) | Parent Portal — view child's attendance (backend + frontend): `GET /api/parent/children/{childId}/attendance` on the existing controller (same `ResolveLinkedChildOrThrow` guard), new `AttendanceService.GetStudentSummaryAsync` (server-owned rate = (Present+Late)/TotalMarked, Absent+Excused count against, null at zero) + `StudentAttendanceSummaryDto`, `ParentAttendanceDto` combining summary + reused `GetStudentHistoryAsync` log; frontend second "Attendance" nav item + `ChildAttendancePage` (rate hero + daily log), shared `ParentChildYearBar` extracted from `ChildGradesPage`, reuses existing attendance status-color map; no new entity/migration | Implemented |
| [specs/20-implement-parent-portal-fees.md](specs/20-implement-parent-portal-fees.md) | Parent Portal — view fee balance / invoices (backend + frontend): `GET /api/parent/children/{childId}/fees` on the existing controller (same `ResolveLinkedChildOrThrow` guard), new `FeeInvoiceService.GetStudentFeeOverviewAsync` (server-owned balance rollup: Billed/Paid/Outstanding/Next-Due + on-the-fly overdue via `OverdueInstallmentIds`) + `StudentFeeSummaryDto`/`StudentFeeOverviewDto`, `IFeeInvoiceRepository.GetIssuedForStudentAndYearWithDetailsAsync` (Issued-only), reuses spec-12 `FeeInvoiceDto` unchanged; frontend fourth "Fees" nav item + `ChildFeesPage` (balance hero + installment schedule + line items), reuses `ParentChildYearBar` + `en-PH` currency; no new entity/migration | Implemented |
